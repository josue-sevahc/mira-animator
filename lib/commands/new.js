...
export default async function newDeck(args) {
  ...
  // 2.5. Copia o editor de deck (modo edição: reordenar slides com a tecla E).
  //      O template já referencia <script src="mira-edit.js">; aqui trazemos o
  //      arquivo pra pasta do deck, deixando o modo edição embutido por padrão.
  const editorSrc = join(MIRA_ROOT, 'templates', 'authoring', 'mira-edit.js');
  if (existsSync(editorSrc)) {
    cpSync(editorSrc, join(dest, 'mira-edit.js'));
  }

  // 2.6. Copia o telestrator (desenho sobre o slide com a tecla P). O template
  //      já referencia <script src="mira-draw.js">; aqui trazemos o arquivo pra
  //      pasta do deck, deixando o desenho ao vivo embutido por padrão.
  const drawSrc = join(MIRA_ROOT, 'templates', 'authoring', 'mira-draw.js');
  if (existsSync(drawSrc)) {
    cpSync(drawSrc, join(dest, 'mira-draw.js'));
  }

  // 2.7. Copia o quadro tático (peças arrastáveis com a tecla T). O template já
  //      referencia <script src="mira-tactics.js">; aqui trazemos o arquivo pra
  //      pasta do deck, deixando o quadro tático embutido por padrão.
  const tacticsSrc = join(MIRA_ROOT, 'templates', 'authoring', 'mira-tactics.js');
  if (existsSync(tacticsSrc)) {
    cpSync(tacticsSrc, join(dest, 'mira-tactics.js'));
  }
