...
export default async function editDeck(args) {
  ...
  // 1. copia os scripts pra pasta do deck (sempre por cima, pra atualizar)
  for (const t of tools) {
    ...
    cpSync(src, join(deckDir, t.file));
  }

  // 2. injeta os <script> antes de </body>, os que ainda não estiverem referenciados
  let html = readFileSync(indexPath, 'utf8');
  const injectedList = [];
  for (const t of tools) {
    if (new RegExp(t.file.replace('.', '\\.')).test(html)) continue;
    const tag =
      `    <!-- ${t.comment} -->\n` +
      `    <script src="${t.file}" defer></script>\n`;
    if (/<\/body>/i.test(html)) {
  ...
  if (injected) writeFileSync(indexPath, html, 'utf8');
  ...
  console.log(`  Scripts: mira-edit.js + mira-draw.js + mira-tactics.js copiados para a pasta do deck`);
